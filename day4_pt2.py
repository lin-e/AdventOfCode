    ...
    for i in range(0, len(parts) - 1):
        for n in parts[i]:
            if n in parts_dict:
                ...
                parts_dict[n] = 1
                #"".join([v[0] for v in sorted(parts_dict.items(), key=lambda kv: (-kv[1], kv[0]))])[:5]
    if "".join([v[0] for v in sorted(parts_dict.items(), key=lambda kv: (-kv[1], kv[0]))])[:5] == parts[len(parts) - 1].split('[')[1].split(']')[0]:
...
def get_sector(name):
    parts = name.split('-')
    return int(parts[len(parts) - 1].split('[')[0])
def shift(char, offset):
    if char == "-":
        return " "
    return chr(97 + (((ord(char) - 97) + offset) % 26)) 
all_names = """nzydfxpc-rclop-qwzhpc-qtylyntyr-769[oshgk]
...
real_names = list()
for i in all_names:
    if not int(is_real(i)) == 0:
        real_names.append(i)
for i in real_names:
    sector = get_sector(i)
    n = list()
    for x in i:
        n.append(shift(x, sector % 26))
    print("".join(n), "[", str(sector), "]")
