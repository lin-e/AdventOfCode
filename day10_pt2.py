class bot:
    ...
    def can_run(self):
        return not (self.chips[0] == None or self.chips[1] == None)
    def higher(self):
        if not self.can_run():
            return -1
        else:
    ...
    def lower(self):
        if not self.can_run():
            ...
            if self.chips[0] > self.chips[1]:
                ...
                return 0
    def get_chip(self, index):
        return self.chips[index]
    def add(self, value):
        if not self.can_run():
            if self.chips[0] == None:
                self.chips[0] = value
            else:
                self.chips[1] = value
    def delete(self, index):
        self.chips[index] = None
class out:
    def __init__(self):
        self.chips = list()
    def add(self, chip):
...
def compare(comparison):
    global search
    return (search == comparison) or (search == comparison[::-1])
def run(cmd):
    global bots
    global bins
    parts = cmd.split()
    ...
    if parts[0] == "value":
        new_val = int(parts[1])
        bot_id = int(parts[5])
        bots[bot_id].add(new_val)
        executed = True
    else:
        bot_id = int(parts[1])
        if bots[bot_id].can_run():
            id_low = int(parts[6])
            id_high = int(parts[11])
            type_low = parts[5]
            type_high = parts[10]
            self_low = bots[bot_id].get_chip(bots[bot_id].lower())
            self_high = bots[bot_id].get_chip(bots[bot_id].higher())
            if compare([self_low, self_high]):
                print(bot_id)
            bots[bot_id].delete(0)
            bots[bot_id].delete(1)
            (bots if (type_low == "bot") else bins)[id_low].add(self_low)
            (bots if (type_high == "bot") else bins)[id_high].add(self_high)
            executed = True
    return executed
input_commands = """bot 49 gives low to bot 118 and high to bot 182
...
bot 96 gives low to bot 10 and high to bot 152""".split('\n')
for i in range(0, 210):
    bots.append(bot())
    bins.append(out())
while len(done) < len(input_commands):
