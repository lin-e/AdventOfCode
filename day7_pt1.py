def supports(name):
    inside = list()
    outside = list()
    new_string = list() # because i dont know regex in python lol
    in_bracket = False
    for x in range(0, len(name)):
        i = name[x]
        if i == "[":
            in_bracket = True
            outside.append("".join(new_string))
            del new_string[:]
        elif i == "]":
            in_bracket = False
            inside.append("".join(new_string))
            del new_string[:]
        else:
            new_string.append(i)
        if x == len(name) - 1:
...
def is_abba(name):
    for i in range(0, len(name) - 3):
        new_key = name[i] + name[i + 1] + name[i + 2] + name[i + 3]
        if new_key[0] == new_key[1]:
            #pass
            continue
        if new_key == new_key[::-1]:
            return True
    return False
all_ip = """xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
